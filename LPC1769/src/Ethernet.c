...
void Ethernet_Init(){
	...
	LPC_EMAC->CLRT =  RETRANSMISSION_MAXIMUM | (COLLISION_WINDOW<<8);

	LPC_EMAC->IPGR = (NON_BACK_TO_BACK_INTER_PACKET_GAP_PART1<<8) | NON_BACK_TO_BACK_INTER_PACKET_GAP_PART2;
	...
	LPC_EMAC->SA0 = UIP_ETHADDR0 | (UIP_ETHADDR1 << 8);
	LPC_EMAC->SA1 = UIP_ETHADDR2 | (UIP_ETHADDR3 << 8);
	LPC_EMAC->SA2 = UIP_ETHADDR4 | (UIP_ETHADDR5 << 8);
...
int Ethernet_ReceiveFrame(char * buffer){
	...
	if(idx != LPC_EMAC->RxProduceIndex){
		...
		char * pointer = (char *) (( int *)LPC_EMAC->RxDescriptor)[idx*2];

		int size = ((int *)LPC_EMAC->RxDescriptor)[idx*2 + 1] & RX_CONTROL_SIZE_MASK;
...
void Ethernet_WriteFrame(char * buffer, int length){
	...
	char * pointer = (char *)((int *)LPC_EMAC->TxDescriptor)[idx*2];

	for(int i = 0; i < length; i++)
		pointer[i] = buffer[i];

	int * aux = &((int *)LPC_EMAC->TxDescriptor)[idx*2 + 1];
	*aux  = length | (1 << 30) | (1<<31) | (1<<29) | (1<<28) | (1<<26);
